...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _forgotPassword() async {
    ...
    // Слушаем deep link — когда придёт passwordRecovery, закрываем окошко
    // и открываем экран смены пароля
    final subscription = Supabase.instance.client.auth.onAuthStateChange.listen((data) {
      if (data.event == AuthChangeEvent.passwordRecovery) {
        if (mounted) {
          Navigator.pop(context); // закрываем окошко
          Navigator.push(
            context,
    ...
    showModalBottomSheet(
      context: context,
      isDismissible: false,      // нельзя закрыть свайпом
      enableDrag: false,         // нельзя закрыть перетаскиванием
          ...
          children: [
            Row(
              ...
              children: [
                ...
                IconButton(
                  onPressed: () {
                    subscription.cancel(); // отписываемся если закрыл вручную
    ...
    ).then((_) => subscription.cancel()); // подстраховка — отписываемся когда окошко закрылось
  }
